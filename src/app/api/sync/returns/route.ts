...
interface MoySkladReturn {
  ...
  demand?: { meta?: { href?: string } } // ссылка на оригинальный чек
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (department) {
      ...
      if (storeIds.length > 0) {
        for (const storeId of storeIds) {
          ...
          const result = await syncReturnsWithFilter(storeFilter, errors)
          totalSynced += result
    ...
    // Если отдел не указан - грузим все возвраты
    if (hasMore) {
      while (hasMore) {
        try {
          const response = await fetch(
            `${MOYSKLAD_API_URL}/entity/retailsalesreturn?filter=${encodeURIComponent(filter)}&limit=${limit}&offset=${offset}&order=moment,desc`,
            {
              headers: {
          ...
          const returns: MoySkladReturn[] = data.rows || []
          ...
          for (const ret of returns) {
            const result = await processReturn(ret, errors)
    ...
    return NextResponse.json(
      { error: 'Ошибка синхронизации возвратов' },
      { status: 500 }
    )
  }
}

async function syncReturnsWithFilter(filter: string, errors: string[]): Promise<number> {
  let synced = 0
  let offset = 0
  const limit = 1000
  let hasMore = true
  ...
  while (hasMore) {
    try {
      const response = await fetch(
        `${MOYSKLAD_API_URL}/entity/retailsalesreturn?filter=${encodeURIComponent(filter)}&limit=${limit}&offset=${offset}`,
        {
          headers: {
            'Authorization': `Bearer ${MOYSKLAD_TOKEN}`,
            'Accept-Encoding': 'gzip',
...
async function processReturn(ret: MoySkladReturn, errors: string[]): Promise<boolean> {
  const ownerId = ret.owner?.meta?.href?.split('/').pop()
  const storeId = ret.retailStore?.meta?.href?.split('/').pop()
  const storeInfo = storeId ? RETAIL_STORES[storeId] : null
  const demandId = ret.demand?.meta?.href?.split('/').pop()
  ...
  const upsertData = {
    ...
    amount: ret.sum / 100,
    return_date: ret.moment.slice(0, 10),
    return_datetime: ret.moment,
    retail_store_id: storeId || null,
    retail_store_name: storeInfo?.name || null,
    original_demand_id: demandId || null,
  }
  ...
  if (error) {
    errors.push(`Ошибка сохранения возврата ${ret.id}: ${error.message}`)
    return false
  }
